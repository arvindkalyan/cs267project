...
class FacebookEgoNetwork:
    ...
    def compute_node2vec_embeddings(self, dimensions=16):
        ...
        return {int(node): torch.tensor(model.wv[node], dtype=torch.float) for node in model.wv.index_to_key}
    ...
    def load_ego_network(self, ego_id):
        ...
        with open(edges_file, "r") as f:
            for line in f:
                node1, node2 = map(int, line.strip().split())
                if node1 not in all_nodes: G.add_node(node1)
                if node2 not in all_nodes: G.add_node(node2)
        self.graph = G
        return G

    def extract_link_features(self, node1, node2):
        if self.graph is None: raise ValueError("No graph loaded.")
        G = self.graph
        cn = len(list(nx.common_neighbors(G, node1, node2)))
        n1, n2 = set(G.neighbors(node1)), set(G.neighbors(node2))
        jaccard = 0
        if len(n1 | n2) > 0: jaccard = len(n1 & n2) / len(n1 | n2)
        aa_index = sum(1 / np.log(G.degree(u)) for u in nx.common_neighbors(G, node1, node2) if G.degree(u) > 1)
    ...
    def generate_training_data(self, test_size=0.2, use_embeddings=True, embedding_dim=16):
        ...
        pos_train, pos_test = train_test_split(positive_examples, test_size=test_size, random_state=RANDOM_SEED)
        neg_train, neg_test = train_test_split(negative_examples, test_size=test_size, random_state=RANDOM_SEED)
        
        X_train, y_train, X_test, y_test = [], [], [], []
        test_edges = pos_test + neg_test
        
        extractor = lambda u, v: self.extract_link_features_with_embeddings(u, v, embeddings) if use_embeddings else self.extract_link_features(u, v)
        
        for edge in pos_train: X_train.append(extractor(edge[0], edge[1])); y_train.append(1)
        for edge in neg_train: X_train.append(extractor(edge[0], edge[1])); y_train.append(0)
        for edge in pos_test: X_test.append(extractor(edge[0], edge[1])); y_test.append(1)
        for edge in neg_test: X_test.append(extractor(edge[0], edge[1])); y_test.append(0)

        return (torch.stack(X_train), torch.tensor(y_train, dtype=torch.float),
                torch.stack(X_test), torch.tensor(y_test, dtype=torch.float),
                test_edges, embeddings)
...
def plot_benchmarks(results):
    ...
    for name, (auc, acc) in results.items():
        if 'Bayesian' in name: model_type = 'Bayesian'
        elif 'SKlearn' in name: model_type = 'Classic ML'
        else: model_type = 'Heuristic'
...
def run_edge_recovery_bayesian(loader, model, guide, ego_id, fraction=0.2, use_embeddings=True, embedding_dim=16):
    ...
    embeddings = temp_loader.compute_node2vec_embeddings(dimensions=embedding_dim) if use_embeddings else None
    extractor = lambda u, v: temp_loader.extract_link_features_with_embeddings(u, v, embeddings) if use_embeddings else temp_loader.extract_link_features(u,v)
    ...
    print(f"{'[Recovery] Bayesian LR':<28} | Recovery Rate: {recovery_rate:.4f} ({recovered_count}/{total_removed})")
...
def run_edge_recovery_heuristic(loader, ego_id, fraction=0.2):
    ...
    heuristic_scores = {
        "Adamic-Adar Index": [s for _, _, s in nx.adamic_adar_index(G_severed, removed_edges)],
        "Jaccard Coefficient": [s for _, _, s in nx.jaccard_coefficient(G_severed, removed_edges)],
        "Preferential Attachment": [s for _, _, s in nx.preferential_attachment(G_severed, removed_edges)],
    }
    
    # Evaluate each heuristic
    for name, scores in heuristic_scores.items():
        scores_np = np.asarray(scores)
        if len(scores_np) == 0: continue
        ...
        total_removed = len(y_pred)
        recovery_rate = recovered_count / total_removed if total_removed > 0 else 0
        print(f"{f'[Recovery] {name}':<28} | Recovery Rate: {recovery_rate:.4f} ({recovered_count}/{total_removed})")

def run_edge_recovery_classic(loader, trained_models, ego_id, fraction=0.2, use_embeddings=True, embedding_dim=16):
...
def main():
    ...
    print(f"Generated {len(X_train)} training examples and {len(X_test)} testing examples.")
    ...
    # Classic ML Models
    classic_ml_scores, trained_classic_models = get_classic_ml_scores(X_train, y_train, X_test)
